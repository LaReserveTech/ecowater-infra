...
const synchronize: Handler = async (_event: any) => {
  ...
  try {
    ...
    for (const decree of decrees) {
      ...
      const eventCreateionQuery = `
        INSERT INTO event_store (stream_id, type, payload, occurred_at) VALUES ($1, $2, $3, $4);
      `
      await client.query(eventCreateionQuery, [decrees[0].id, 'decree_creation', null, new Date()])
    }
