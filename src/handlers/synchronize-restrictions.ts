...
const synchronize: Handler = async (_event: any) => {
  ...
  try {
    ...
    for (const restriction of restrictions) {
      const res = await client.query('SELECT id FROM decree WHERE external_id = $1', [
      ...
      const parsedFromHour = parseFloat(restriction['heure_debut'])
      const fromHour = !Number.isNaN(parsedFromHour) ? Math.floor(parsedFromHour) : null
      const parsedToHour = parseFloat(restriction['heure_fin'])
      const toHour = !Number.isNaN(parsedToHour) ? Math.ceil(parsedToHour) : null
